...
let VIS_CACHE = { t: 0, list: null };
async function discoverVisionModels() {
  if (VIS_CACHE.list && Date.now() - VIS_CACHE.t < 6 * 3600 * 1000) return VIS_CACHE.list;
  try {
    const r = await fetch(AI_BASE + "/models", { headers: { "Authorization": "Bearer " + AI_KEY } });
    const d = await r.json();
    const list = (d.data || []).filter(m => {
      const a = m.architecture || {};
      const mods = a.input_modalities || a.modality || [];
      const img = Array.isArray(mods) ? mods.includes("image") : String(mods).indexOf("image") > -1;
      const free = String(m.id || "").endsWith(":free");
      return img && free;
    }).map(m => m.id);
    if (list.length) { VIS_CACHE = { t: Date.now(), list: list }; return list; }
  } catch (e) {}
  return null;
}
function visionCandidates(discovered) {
  if (process.env.AI_VISION_MODEL) {
    const envList = String(process.env.AI_VISION_MODEL).split(",").map(s => s.trim()).filter(Boolean);
    if (envList.length) return envList;
  }
  if (discovered && discovered.length) {
    const score = x => (/gemini/.test(x) ? 0 : /qwen.*vl/.test(x) ? 1 : /vision|llama-4/.test(x) ? 2 : /mistral/.test(x) ? 3 : 4);
    return discovered.slice().sort((a, b) => score(a) - score(b)).slice(0, 6);
  }
  return VISION_FALLBACK;
}
app.get("/api/vision/models", async (req, res) => {
  if (!adminOk(req)) return res.status(401).json({ error: "Ruxsat yo'q" });
  const disc = await discoverVisionModels();
  res.json({ env: process.env.AI_VISION_MODEL || null, discovered: disc || [], willTry: visionCandidates(disc) });
});
app.post("/api/vision", async (req, res) => {
  if (!rateOk("vis:" + clientIp(req), 8, 60000)) return res.status(429).json({ error: "Juda ko'p so'rov, biroz kuting" });
  ...
  if (img.length > 6500000) return res.status(400).json({ error: "Rasm juda katta" });
  const prompt = String(b.prompt || "").slice(0, 1500) || "Rasmdagi masalani o'qib, bosqichma-bosqich yech.";
  ...
  try {
    ...
    const discovered = await discoverVisionModels();
    const models = visionCandidates(discovered);
    const msgs = [];
    ...
    for (const model of models) {
      try {
        ...
        const data = await r.json();
        if (!r.ok) { tried.push(model + " -> " + String((data.error && (data.error.message || data.error)) || r.status).slice(0, 60)); continue; }
        const text = ((data.choices && data.choices[0] && data.choices[0].message && data.choices[0].message.content) || "").trim();
        if (text) return res.json({ content: [{ type: "text", text: text }], model: model });
        tried.push(model + " -> bo'sh javob");
      } catch (e) { tried.push(model + " -> ulanmadi"); }
    }
    return res.status(502).json({ error: "Vision modellari javob bermadi", tried: tried });
